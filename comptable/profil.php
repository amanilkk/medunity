...
if ($user['role_name'] == 'medecin') {
    $stmt = $database->prepare("SELECT * FROM doctors WHERE user_id = ?");
    $stmt->bind_param('i', $user_id);
    $stmt->execute();
    $additional_info = $stmt->get_result()->fetch_assoc();
} elseif ($user['role_name'] == 'patient') {
    $stmt = $database->prepare("SELECT * FROM patients WHERE user_id = ?");
    $stmt->bind_param('i', $user_id);
    $stmt->execute();
    $additional_info = $stmt->get_result()->fetch_assoc();
}
...
// Congés restants
$remaining_leaves = getRemainingLeaveDays($database, $user_id, $current_year);
...
<head>
    ...
    <title>Mon profil - Comptable</title>
                    ...
                    <div class="card-body">
                        ...
                        <div class="profile-stats">
                            <div class="profile-stat">
                                <div class="value"><?php echo $attendance_stats['present_days'] ?? 0; ?></div>
                                <div class="label">Présences ce mois</div>
                            </div>
                            <div class="profile-stat">
                                <div class="value"><?php echo $remaining_leaves; ?></div>
                                <div class="label">Congés restants</div>
                            </div>
                            <div class="profile-stat">
                                <div class="value"><?php echo ($attendance_stats['total_hours'] ?? 0); ?>h</div>
